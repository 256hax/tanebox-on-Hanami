...
module Gnote
  class Application < Hanami::Application
    configure do
      ...
      #
      # 記述する順番に気をつけること。You write 'helpers' first.
      # https://github.com/hanami/hanami.github.io/issues/112
      load_paths << [
        'helpers',
        'controllers',
        'views'
      ...
      #
      # sessions :cookie, secret: ENV['GNOTE_SESSIONS_SECRET']
      ...
      #
      layout :application # It will load Gnote::Views::ApplicationLayout
      ...
      # Google fonts
      font_src_google_fonts = %{https://fonts.gstatic.com https://cdnjs.cloudflare.com}

      # Twitter Widgets
      form_action_twitter = %{https://platform.twitter.com/ https://syndication.twitter.com}
      script_src_twitter = %{http://platform.twitter.com/ https://platform.twitter.com/ https://cdn.syndication.twimg.com}
      img_src_twitter = %{http://abs.twimg.com}
      frame_src_twitter = %{https://platform.twitter.com/ https://platform.twitter.com/ https://syndication.twitter.com/}

      security.content_security_policy %{
        form-action 'self' #{form_action_twitter};
        frame-ancestors 'self';
        base-uri 'self';
        default-src 'none';
        script-src 'self' #{script_src_twitter};
        connect-src 'self';
        img-src 'self' https: data: #{img_src_twitter};
        style-src 'self' 'unsafe-inline' https:;
        font-src 'self' #{font_src_google_fonts};
        object-src 'none';
        plugin-types application/pdf;
        child-src 'self';
        frame-src 'self' #{frame_src_twitter};
        media-src 'self'
      }
      ...
      # Configure the code that will yield each time Gnote::Action is included
      ...
      # Configure the code that will yield each time Gnote::View is included
      # This is useful for sharing common functionality
      #
      # See: http://www.rubydoc.info/gems/hanami-view#Configuration
      view.prepare do
        include Hanami::Helpers
        include Gnote::Assets::Helpers
        include Gnote::Helpers::DateFormat
      end
    end
